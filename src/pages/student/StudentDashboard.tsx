  ...
  return (
    <motion.div initial="hidden" animate="visible" variants={cv} className="min-h-screen bg-[#F5F5F0] lg:flex">
      ...
      <main className="pt-14 lg:pt-0 lg:ml-80 flex-1 p-4 sm:p-6 md:p-10 lg:p-16 xl:p-20 relative overflow-hidden">
        ...
        {/* Error */}
        {error && (
          <div className="mb-6 bg-red-50 border border-red-200 rounded-2xl p-4 text-sm font-bold text-[#D4A373]">{error}</div>
        )}
        ...
        <header className="mb-10 lg:mb-16 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6 relative z-10">
          ...
          <motion.div variants={ci} className="flex gap-4 items-center">
            <div className="bg-white p-4 sm:p-5 rounded-2xl border border-[#1A1A1A]/5 shadow-xl flex items-center gap-4 group hover:-translate-y-2 transition-transform cursor-pointer">
              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-[#DE0002]/10 flex items-center justify-center text-[#DE0002] group-hover:bg-[#DE0002] group-hover:text-white transition-all shrink-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 lg:gap-8 relative z-10">
          ...
          <motion.div variants={ci} className="col-span-1 md:col-span-12 lg:col-span-8 bg-[#1A1A1A] rounded-[2.5rem] p-8 sm:p-10 lg:p-14 text-white relative overflow-hidden shadow-2xl border border-white/5 group">
            <div className="absolute top-0 right-0 w-64 h-64 bg-[#DE0002]/20 rounded-full blur-[80px] -translate-y-1/2 translate-x-1/2 group-hover:bg-[#DE0002]/40 transition-all duration-1000" />
            <div className="relative z-10 flex flex-col h-full justify-between">
              <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                    ...
                    {loading
                      ...
                      : <h3 className="text-3xl sm:text-4xl lg:text-5xl font-black tracking-tighter uppercase leading-none">
                          {profile?.current_level ? `Level ${profile.current_level}` : 'Not Enrolled'}<br />
                          <span className="text-[#DE0002]">
                            {(profile as any)?.group?.name ? `Group ${(profile as any).group.name}` : 'Programme.'}
                          </span>
                        </h3>
                    }
                  </div>
                ...
                <div className="mt-8 p-8 bg-white/5 rounded-3xl border border-white/10 shadow-2xl space-y-6 relative overflow-hidden group/card">
                   <div className="absolute top-0 right-0 w-32 h-32 bg-[#D4A373]/20 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 opacity-0 group-hover/card:opacity-100 transition-opacity" />
                   
                   <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 relative z-10">
                      <div className="flex items-center gap-4">
                         <div className="w-12 h-12 rounded-2xl bg-[#D4A373] flex items-center justify-center text-white shadow-lg shadow-[#D4A373]/20">
          ...
          <motion.div variants={ci} className="col-span-1 md:col-span-12 lg:col-span-4 bg-white rounded-[2.5rem] p-8 sm:p-10 lg:p-12 border border-[#1A1A1A]/5 shadow-sm space-y-8 group hover:shadow-2xl transition-all">
                      ...
                      {latestPendingReview ? (
                        ...
                        <>
                          ...
                          <div className="h-2 w-full bg-[#F5F5F0] rounded-full overflow-hidden shadow-inner">
                            <div
                              className="h-full bg-[#D4A373] rounded-full shadow-[0_0_12px_rgba(198,40,40,0.4)] transition-all duration-1000"
                              style={{ width: `${latestResult.score ?? 0}%` }}
                            />
                          </div>
                          <span className={`text-xs font-black uppercase px-3 py-1 rounded-full ${latestResult.passed ? 'bg-green-50 text-green-600' : 'bg-red-50 text-[#D4A373]'}`}>
            ...
            <button onClick={() => navigate('/student/results')} className="w-full flex items-center justify-between group/btn pt-2">
              <span className="text-[10px] font-black uppercase tracking-widest text-[#1A1A1A] group-hover/btn:text-[#D4A373] transition-colors">View All Results</span>
              <FiArrowRight className="w-4 h-4 group-hover/btn:translate-x-2 transition-transform text-[#D4A373]" />
            </button>
          </motion.div>
          ...
          <motion.div variants={ci} className="col-span-1 md:col-span-6 lg:col-span-5 bg-white rounded-[2.5rem] p-8 lg:p-12 border border-[#1A1A1A]/5 shadow-sm hover:shadow-2xl transition-all group">
            <div className="flex justify-between items-center mb-8">
              ...
              <FiFileText className="text-[#D4A373] w-5 h-5" />
            </div>
            {loading
                ...
                : <div className="space-y-4">
                    {pendingAssignments.map(a => (
                      <div
                        ...
                        className="flex items-center justify-between p-4 bg-[#F5F5F0] rounded-2xl hover:bg-[#D4A373]/5 cursor-pointer transition-all group/item"
                      >
                        <div className="min-w-0">
                          <p className="font-black text-sm text-[#1A1A1A] truncate group-hover/item:text-[#D4A373] transition-colors">{a.title}</p>
                          <p className="text-[10px] font-black text-[#1A1A1A]/30 uppercase tracking-wider mt-0.5">Due {formatDeadline(a.deadline)}</p>
                        </div>
                        <FiArrowRight className="w-4 h-4 text-[#1A1A1A]/20 group-hover/item:text-[#D4A373] group-hover/item:translate-x-1 transition-all shrink-0 ml-3" />
                      </div>
                    ))}
                  </div>
            }
            <button onClick={() => navigate('/student/assignments')} className="mt-6 w-full flex items-center justify-between group/btn">
              <span className="text-[10px] font-black uppercase tracking-widest text-[#1A1A1A] group-hover/btn:text-[#D4A373] transition-colors">All Assignments</span>
              <FiArrowRight className="w-4 h-4 group-hover/btn:translate-x-2 transition-transform text-[#D4A373]" />
            </button>
          </motion.div>
          ...
          <motion.div variants={ci} className="col-span-1 md:col-span-12 lg:col-span-4 bg-white rounded-[2.5rem] p-8 lg:p-12 border border-[#1A1A1A]/5 shadow-sm hover:shadow-2xl transition-all group">
            <div className="flex justify-between items-center mb-8">
              ...
              <FiBookOpen className="text-[#D4A373] w-5 h-5" />
            </div>
            {loading
                ...
                : <div className="space-y-3">
                    {materials.slice(0, 4).map(m => (
                      <button
                        ...
                        className="flex items-center gap-4 p-4 bg-[#F5F5F0] rounded-2xl hover:bg-[#D4A373]/5 cursor-pointer transition-all group/item"
                      >
                        <div className="w-9 h-9 rounded-xl bg-[#D4A373]/10 flex items-center justify-center shrink-0">
                          <FiBookOpen className="w-4 h-4 text-[#D4A373]" />
                        </div>
                        <p className="font-black text-sm text-[#1A1A1A] truncate group-hover/item:text-[#D4A373] transition-colors">{m.title}</p>
                      </button>
                    ))}
                  </div>
            }
            <button onClick={() => navigate('/student/courses')} className="mt-6 w-full flex items-center justify-between group/btn">
              <span className="text-[10px] font-black uppercase tracking-widest text-[#1A1A1A] group-hover/btn:text-[#D4A373] transition-colors">All Materials</span>
              <FiArrowRight className="w-4 h-4 group-hover/btn:translate-x-2 transition-transform text-[#D4A373]" />
            </button>
          </motion.div>
          ...
          <motion.div variants={ci} className="col-span-1 md:col-span-12 lg:col-span-8 bg-white rounded-[2.5rem] p-8 lg:p-12 border border-[#1A1A1A]/5 shadow-sm hover:shadow-2xl transition-all">
            ...
            {latestReviewed ? (
              <div className="space-y-3">
                <p className="text-sm font-black text-[#1A1A1A]">Grade: <span className="text-[#D4A373]">{latestReviewed.grade ?? 0}%</span></p>
                ...
                <button onClick={() => navigate('/student/assignments')} className="text-[10px] font-black uppercase tracking-widest text-[#D4A373] hover:underline">
                  Open assignments
                </button>
              </div>
            ) : (
