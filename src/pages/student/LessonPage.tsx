  ...
  return (
    ...
    >
      ...
      <header className="fixed top-0 left-0 w-full h-24 bg-white/80 backdrop-blur-2xl border-b border-[#1A1A1A]/5 px-12 z-[100] flex items-center justify-between group">
        <div className="flex items-center gap-12">
          <button 
            ...
            className="flex items-center gap-4 text-[#1A1A1A]/40 hover:text-[#F97316] font-black text-[10px] uppercase tracking-[0.5em] transition-all group/back"
          >
        ...
        <div className="flex items-center gap-10">
           <div className="flex items-center gap-4 bg-[#F5F5F0] px-6 py-2 rounded-full border border-[#1A1A1A]/5">
              <Clock className="text-[#F97316] w-5 h-5 shadow-sm" />
              <span className="text-[10px] font-black text-[#1A1A1A]/40 uppercase tracking-widest leading-none">42:12 Time Remaining</span>
           </div>
           <button className="bg-[#1A1A1A] text-white px-8 py-4 rounded-2xl font-black text-[10px] uppercase tracking-[0.3em] hover:bg-[#F97316] transition-all shadow-2xl active:scale-95">
              Checkpoint Status
           </button>
        </div>
      </header>
      ...
      <main className="max-w-[1440px] mx-auto pt-48 px-12 lg:px-24 grid grid-cols-12 gap-16 relative z-10">
        <div className="absolute top-0 right-0 w-[40vw] h-[40vw] bg-[#F97316]/[0.02] rounded-full blur-[100px] pointer-events-none -translate-y-1/2 translate-x-1/2 -z-0"></div>
        ...
        <div className="col-span-12 lg:col-span-8 space-y-16">
          <motion.section variants={containerVariants} className="bg-white rounded-[4rem] overflow-hidden shadow-2xl border border-[#1A1A1A]/5 group relative h-[600px] flex items-center justify-center cursor-pointer">
             ...
             <div className="absolute inset-0 bg-[#F97316]/10 group-hover:bg-transparent transition-all duration-700"></div>
             <div className="absolute inset-0 bg-gradient-to-t from-[#1A1A1A] via-transparent to-transparent"></div>
             
             <button className="relative z-10 w-24 h-24 bg-white text-[#F97316] rounded-full flex items-center justify-center shadow-[0_40px_80px_rgba(255,255,255,0.3)] hover:scale-110 transition-transform active:scale-95 group/play border-4 border-[#F97316]/10">
             ...
             <div className="absolute bottom-12 left-12 right-12 flex justify-between items-end">
                <div className="space-y-4">
                   ...
                   <div className="flex gap-4 items-center">
                      <Layers className="text-[#F97316] w-5 h-5 shadow-sm" />
                ...
                <div className="flex gap-4">
                   <button className="w-12 h-12 rounded-xl bg-white/10 backdrop-blur-xl border border-white/20 flex items-center justify-center text-white hover:bg-[#F97316] transition-all shadow-lg active:scale-95">
          ...
          <motion.section variants={containerVariants} className="space-y-12">
             ...
             <div className="bg-white p-12 lg:p-20 rounded-[4rem] shadow-2xl border border-[#1A1A1A]/5 relative overflow-hidden group">
                <div className="absolute bottom-0 right-0 w-64 h-64 bg-[#F97316]/[0.02] rounded-full blur-3xl -z-0"></div>
                <div className="space-y-12 relative z-10">
                   <p className="text-2xl font-medium text-[#1A1A1A]/80 leading-relaxed font-body italic underline decoration-[#F97316]/10 underline-offset-8">
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                      <div className="p-10 bg-[#F5F5F0] rounded-[3rem] border-l-[10px] border-[#F97316] space-y-4 hover:shadow-xl transition-all shadow-sm">
                      ...
                      <div className="p-10 bg-[#1A1A1A] rounded-[3rem] border-l-[10px] border-[#D4A373] space-y-4 shadow-2xl hover:scale-105 transition-transform group">
                        ...
                        <p className="text-xl font-black text-white uppercase tracking-tighter leading-tight italic">"Ich hätte Zeit."<br/><span className="text-[#F97316]">(I would have time.)</span></p>
                      </div>
                   </div>
        ...
        <div className="col-span-12 lg:col-span-4 space-y-12">
           <motion.div variants={containerVariants} className="bg-white p-12 rounded-[4rem] shadow-2xl border border-[#1A1A1A]/5 space-y-12 group hover:shadow-xl transition-all">
              <div className="flex justify-between items-center">
                 ...
                 <Zap className="text-[#F97316] w-6 h-6 animate-pulse" />
              </div>
                 ...
                 {[
                   ...
                   <div key={i} className={`p-6 rounded-[2.5rem] flex items-center justify-between transition-all group/item shadow-sm cursor-pointer ${item.status === 'active' ? 'bg-[#1A1A1A] text-white shadow-2xl -translate-x-2' : 'bg-[#F5F5F0] text-[#1A1A1A]/20 hover:bg-white border border-[#1A1A1A]/5'}`}>
                      <div className="flex items-center gap-6">
                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${item.status === 'active' ? 'bg-[#F97316] text-white' : item.status === 'complete' ? 'bg-green-100 text-green-600' : 'bg-white text-stone-300'}`}>
                      ...
                      <ChevronRight className={`w-6 h-6 transition-all ${item.status === 'active' ? 'text-[#F97316] translate-x-1' : 'opacity-0'}`} />
                   </div>
                 ))}
              </div>
           </motion.div>
           ...
           <motion.div variants={containerVariants} className="bg-[#1A1A1A] p-12 rounded-[4rem] text-white shadow-2xl relative overflow-hidden group hover:shadow-[0_40px_80px_rgba(26,26,26,0.3)] transition-all flex flex-col justify-between h-96">
              <div className="absolute top-0 right-0 w-32 h-32 bg-[#F97316]/20 rounded-full blur-2xl group-hover:bg-[#F97316]/40 transition-all duration-700"></div>
              ...
              <button className="w-full py-6 bg-[#F97316] text-white rounded-[2rem] font-black text-xs uppercase tracking-[0.4em] shadow-2xl hover:bg-[#a20513] transition-all relative overflow-hidden group/btn mt-10 shadow-lg">
      ...
      <footer className="fixed bottom-0 left-0 w-full h-32 bg-white/95 backdrop-blur-3xl border-t border-[#1A1A1A]/5 px-12 lg:px-24 flex items-center justify-between z-[100] shadow-2xl">
            ...
            <div className="flex flex-col gap-2">
               ...
               <div className="h-2 w-80 bg-[#F5F5F0] rounded-full overflow-hidden border border-[#1A1A1A]/5 shadow-inner">
                  <motion.div 
                    ...
                    className="h-full bg-[#F97316] rounded-full shadow-[0_0_12px_rgba(198,40,40,0.4)]"
                  ></motion.div>
               </div>
            </div>
         </div>
         ...
         <div className="flex items-center gap-12">
            <div className="flex gap-4">
               <button className="w-16 h-16 rounded-2xl bg-[#F5F5F0] flex items-center justify-center text-[#1A1A1A]/40 hover:bg-[#F97316] hover:text-white transition-all shadow-sm active:scale-95 group/prev">
                  <ChevronLeft className="w-8 h-8 group-hover/prev:-translate-x-1 transition-transform" />
               </button>
               <button className="w-16 h-16 rounded-2xl bg-[#F5F5F0] flex items-center justify-center text-[#1A1A1A]/40 hover:bg-[#F97316] hover:text-white transition-all shadow-sm active:scale-95 group/next">
                  <ChevronRight className="w-8 h-8 group-hover/next:translate-x-1 transition-transform" />
               </button>
            </div>
            <button className="px-16 py-6 bg-[#1A1A1A] text-white rounded-2xl font-black text-xs uppercase tracking-[0.4em] shadow-2xl hover:bg-[#F97316] transition-all active:scale-[0.98] shadow-lg">
               Finalize Phase
            </button>
         </div>
      </footer>
