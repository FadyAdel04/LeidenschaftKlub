    ...
    <motion.div initial="hidden" animate="visible" variants={cv} className="min-h-screen bg-[#F5F5F0] lg:flex">
      ...
      <main className="pt-14 lg:pt-0 lg:ml-80 flex-1 p-4 sm:p-6 md:p-10 lg:p-16 xl:p-20 relative overflow-hidden">
        <motion.header variants={ci} className="mb-10 flex flex-col sm:flex-row items-start sm:items-end justify-between gap-6 relative z-10">
          <div>
            <h1 className="text-4xl sm:text-5xl md:text-6xl font-black tracking-tighter text-[#1A1A1A] leading-none uppercase mb-3">Manage<br/><span className="text-[#F97316]">Materials.</span></h1>
          ...
          <button onClick={() => { setShowForm(p => !p); setFormError(''); }}
            className="flex items-center gap-3 bg-[#1A1A1A] text-white px-7 py-4 rounded-2xl font-black text-sm uppercase tracking-wider hover:bg-[#F97316] transition-all active:scale-95 shadow-lg shrink-0">
        ...
        <AnimatePresence>
          ...
          {error && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="mb-6 flex items-center gap-3 bg-red-50 border border-red-200 rounded-2xl p-4 relative z-10"><AlertCircle className="w-4 h-4 text-[#F97316] shrink-0" /><p className="text-xs font-bold text-[#F97316]">{error}</p></motion.div>}
        </AnimatePresence>
          ...
          {showForm && (
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
              ...
              <div className="flex items-center justify-between mb-6">
                <h3 className="font-black text-[#1A1A1A] tracking-tighter uppercase text-lg flex items-center gap-3"><UploadCloud className="w-5 h-5 text-[#F97316]" /> New Material</h3>
                <button onClick={() => setShowForm(false)} className="p-2 rounded-xl bg-[#F5F5F0] text-[#1A1A1A]/40 hover:text-[#F97316]"><X className="w-4 h-4" /></button>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-6">
                <div className="space-y-2">
                  ...
                  <input type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Grammar B1 Unit 3"
                    className="w-full px-5 py-3.5 bg-[#F5F5F0] rounded-2xl font-black text-sm text-[#1A1A1A] placeholder:text-[#1A1A1A]/20 outline-none focus:ring-4 focus:ring-[#F97316]/10 transition-all" />
                </div>
                <div className="space-y-2">
              ...
              <div onClick={() => leRef.current?.click()}
                className={`border-4 border-dashed rounded-[2rem] p-8 flex flex-col items-center cursor-pointer transition-all group mb-6 ${file ? 'border-[#F97316]/40 bg-[#F97316]/5' : 'border-[#1A1A1A]/10 hover:border-[#F97316]/30'}`}>
                {file
                  ? <div className="flex items-center gap-3"><Book className="w-5 h-5 text-[#F97316]" /><p className="font-black text-[#F97316] text-sm">{file.name}</p>
                      <button onClick={e => { e.stopPropagation(); setFile(null); }} className="w-6 h-6 rounded-full bg-[#F97316]/10 flex items-center justify-center hover:bg-[#F97316] hover:text-white"><X className="w-3 h-3" /></button></div>
                  : <><UploadCloud className="w-10 h-10 text-[#1A1A1A]/10 group-hover:text-[#F97316] mb-3 group-hover:-translate-y-2 transition-all" />
                      <p className="font-black text-[#1A1A1A] uppercase tracking-tighter">Drop or <span className="text-[#F97316]">browse</span></p>
              ...
              {formError && <div className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-2xl p-4 mb-6"><AlertCircle className="w-4 h-4 text-[#F97316] shrink-0" /><p className="text-xs font-bold text-[#F97316]">{formError}</p></div>}
              <div className="flex gap-3 justify-end">
                ...
                <button onClick={handleUpload} disabled={uploading}
                  className="flex items-center gap-2 px-8 py-3 bg-[#F97316] text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:shadow-xl transition-all active:scale-95 disabled:opacity-60">
            ...
            : <motion.div variants={cv} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 relative z-10">
                {materials.map(m => (
                  <motion.div key={m.id} variants={ci} whileHover={{ y: -4 }}
                    ...
                    <div className="w-12 h-12 rounded-2xl bg-[#F97316]/10 flex items-center justify-center mb-4 group-hover:bg-[#F97316] transition-all">
                      <Book className="w-6 h-6 text-[#F97316] group-hover:text-white transition-colors" />
                    </div>
                    <span className="text-[8px] font-black uppercase tracking-[0.4em] text-[#D4A373] italic">Level {m.levels?.name ?? '—'}</span>
                    <h3 className="text-base font-black text-[#1A1A1A] tracking-tighter uppercase leading-tight mt-1 mb-3 line-clamp-2 group-hover:text-[#F97316] transition-colors">{m.title}</h3>
                    ...
                    <div className="flex flex-col gap-2">
                      <button
                        ...
                        className="flex-1 flex items-center justify-center gap-2 bg-[#1A1A1A] text-white py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-[#F97316] transition-all active:scale-95">
                        <PlayCircle className="w-3.5 h-3.5" /> Preview
                      </button>
                      <button
                        type="button"
                        onClick={() => openAssignModal(m)}
                        className="flex-1 flex items-center justify-center gap-2 bg-[#F97316] text-white py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-[#1A1A1A] transition-all active:scale-95 shadow-md shadow-[#F97316]/20">
                        <Plus className="w-3.5 h-3.5" /> Assign to Students
                      </button>
                      <div className="flex items-center gap-2 mt-1">
                        <button onClick={() => handleDelete(m.id)} disabled={deleting === m.id}
                          className="flex-1 h-10 rounded-xl bg-[#F5F5F0] flex items-center justify-center text-[#1A1A1A]/30 hover:bg-red-50 hover:text-[#F97316] transition-all active:scale-95 disabled:opacity-60">
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[110] flex items-center justify-center p-4">
              ...
              <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }}
                ...
                <div className="flex items-center justify-between mb-8">
                  ...
                  <button onClick={() => setEditing(null)} className="w-11 h-11 rounded-2xl bg-[#F5F5F0] text-[#1A1A1A]/40 hover:text-[#F97316] flex items-center justify-center transition-all"><X className="w-5 h-5" /></button>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
                  <div className="space-y-2">
                    <label className="text-[9px] font-black uppercase tracking-[0.4em] text-[#D4A373] ml-1 block">Title</label>
                    <input value={editTitle} onChange={e => setEditTitle(e.target.value)} placeholder="Title"
                      className="w-full px-5 py-3.5 bg-[#F5F5F0] rounded-2xl font-black text-sm text-[#1A1A1A] outline-none border border-transparent focus:border-[#F97316]/20 transition-all" />
                  </div>
                  <div className="space-y-2">
                     <label className="text-[9px] font-black uppercase tracking-[0.4em] text-[#D4A373] ml-1 block">Level</label>
                ...
                <div onClick={() => editleRef.current?.click()}
                  className={`border-4 border-dashed rounded-[2rem] p-8 flex flex-col items-center cursor-pointer transition-all group mb-8 ${editFile ? 'border-[#F97316]/40 bg-[#F97316]/5' : 'border-[#1A1A1A]/10 hover:border-[#F97316]/30'}`}>
                  {editFile 
                    ? <div className="flex items-center gap-3"><Book className="w-5 h-5 text-[#F97316]" /><p className="font-black text-[#F97316] text-sm">{editFile.name}</p></div>
                    : <><UploadCloud className="w-8 h-8 text-[#1A1A1A]/10 group-hover:text-[#F97316] mb-2 transition-all" /><p className="font-black text-[10px] text-[#1A1A1A]/30 uppercase tracking-widest">Replace file (optional)</p></>
                  }
                </div>
                {formError && <p className="mb-6 text-xs font-black text-[#F97316] bg-red-50 p-4 rounded-2xl border border-red-100">{formError}</p>}
                <div className="flex justify-end gap-3">
                  ...
                  <button onClick={handleUpdate} disabled={updating}
                    className="inline-flex items-center gap-2 px-8 py-3 bg-[#F97316] text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:shadow-xl transition-all disabled:opacity-60">
                ...
                <div className="p-8 lg:p-10">
                  <div className="flex items-center justify-between mb-8">
                    <div>
                      ...
                      <p className="text-[10px] font-black uppercase tracking-[0.4em] text-[#F97316] italic">{assigningMaterial?.title}</p>
                    </div>
                    <button onClick={() => setAssignmentModalOpen(false)} className="w-11 h-11 rounded-2xl bg-[#F5F5F0] text-[#1A1A1A]/40 hover:text-[#F97316] flex items-center justify-center transition-all"><X className="w-5 h-5" /></button>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center justify-between p-5 bg-[#F5F5F0] rounded-2xl border border-[#1A1A1A]/5">
                        ...
                        <button onClick={() => setVisibility(!visibility)}
                          className={`w-14 h-7 rounded-full p-1 transition-all ${visibility ? 'bg-[#F97316]' : 'bg-[#1A1A1A]/10'}`}>
                    ...
                    <div className="space-y-3 flex flex-col h-[300px]">
                      <div className="flex items-center justify-between px-1">
                        ...
                        <button onClick={toggleSelectAll} className="text-[9px] font-black uppercase tracking-widest text-[#F97316] hover:underline transition-all">
                      ...
                      <div className="flex-1 overflow-y-auto bg-[#F5F5F0] rounded-2xl p-4 space-y-2 border border-[#1A1A1A]/5">
                        {assignmentLoading && <div className="flex items-center justify-center h-full"><Loader className="w-5 h-5 animate-spin text-[#F97316]" /></div>}
                        ...
                        {studentsInLevel.map(s => (
                          <label key={s.id} className="flex items-center gap-3 p-3 bg-white rounded-xl border border-[#1A1A1A]/5 cursor-pointer hover:border-[#F97316]/30 transition-all group">
                            <input type="checkbox" checked={selectedStudentIds.includes(s.id)} 
                              ...
                              className="w-4 h-4 rounded border-[#1A1A1A]/10 text-[#F97316] focus:ring-[#F97316]/20" />
                      ...
                      <p className="text-[9px] font-black uppercase tracking-widest text-[#F97316] text-center mt-2">
                  ...
                  <div className="flex gap-4 justify-end">
                    ...
                    <button onClick={handleAssign} disabled={assignmentLoading || selectedStudentIds.length === 0}
                      className="flex items-center gap-3 px-10 py-4 bg-[#F97316] text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:shadow-2xl hover:shadow-[#F97316]/30 transition-all active:scale-95 disabled:opacity-50 disabled:shadow-none">
