...
import { FiUsers, FiBook, FiFileText, FiAward, FiTrendingUp, FiArrowRight, FiAlertCircle } from 'react-icons/fi';
...
export default function AdminDashboard() {
  ...
  const [stats,   setStats]   = useState({ students: 0, materials: 0, assignments: 0, exams: 0, avgScore: 0 });
  ...
  const kpis = [
    ...
    { label: 'Materials',        val: loading ? '—' : stats.materials,   icon: FiBook,     trend: 'Uploaded' },
    ...
    <motion.div initial="hidden" animate="visible" variants={cv} className="min-h-screen bg-[#F5F5F0] lg:flex">
      ...
      <main className="pt-14 lg:pt-0 lg:ml-80 flex-1 p-4 sm:p-6 md:p-10 lg:p-16 xl:p-20 relative overflow-hidden">
        ...
        <motion.header variants={ci} className="mb-10 lg:mb-14 relative z-10">
          <div className="flex items-center gap-3 text-[#C62828] text-[10px] font-black uppercase tracking-[0.5em] mb-3">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-black tracking-tighter text-[#1A1A1A] leading-none uppercase">
            Control<br /><span className="text-[#C62828]">Panel.</span>
          </h1>
        </motion.header>
          ...
          {kpis.map((k, i) => (
            <div key={i} className="bg-white rounded-4xl p-6 lg:p-8 border border-[#1A1A1A]/5 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all group cursor-default">
              <div className="flex justify-between items-start mb-4 lg:mb-6">
                ...
                <span className="text-[9px] font-black uppercase tracking-widest text-[#C62828] italic">{k.trend}</span>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 relative z-10">
          ...
          <motion.div variants={ci} className="col-span-1 lg:col-span-8 bg-white rounded-[2.5rem] border border-[#1A1A1A]/5 shadow-sm overflow-hidden hover:shadow-2xl transition-all">
            <div className="p-8 lg:p-10 flex items-center justify-between border-b border-[#1A1A1A]/5">
              ...
              <button onClick={() => navigate('/admin/students')} className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-[#C62828] hover:underline group">
                ...
                : <div className="divide-y divide-[#1A1A1A]/5">
                    {recent.map(s => (
                      <div key={s.id} className="flex items-center gap-5 px-8 py-5 hover:bg-[#F5F5F0]/50 transition-all group">
                        <div className="w-11 h-11 rounded-xl bg-[#C62828] flex items-center justify-center text-white font-black text-sm shrink-0">
                        ...
                        <span className="text-[9px] font-black uppercase px-3 py-1.5 rounded-full bg-[#C62828]/5 text-[#C62828] shrink-0">
          ...
          <motion.div variants={ci} className="col-span-1 lg:col-span-4 bg-[#1A1A1A] rounded-[2.5rem] p-8 lg:p-10 text-white relative overflow-hidden group shadow-2xl border border-white/5">
            <div className="absolute top-0 right-0 w-40 h-40 bg-[#C62828]/20 rounded-full -translate-y-1/2 translate-x-1/2 blur-[60px] group-hover:bg-[#C62828]/40 transition-all" />
            <div className="relative z-10 mb-8">
              <div className="flex items-center gap-3 mb-2">
                <FiTrendingUp className="w-5 h-5 text-[#C62828]" />
            ...
            <div className="relative z-10 mt-10 pt-8 border-t border-white/5 grid grid-cols-2 gap-3">
              {[
                ...
                <button
                  ...
                  className="px-4 py-3 bg-white/5 hover:bg-[#C62828]/20 rounded-2xl text-[9px] font-black uppercase tracking-widest text-white/40 hover:text-white transition-all active:scale-95"
                >
                  {link.label}
                </button>
              ))}
          ...
          {/* Avg Score */}
          <motion.div variants={ci} className="col-span-1 lg:col-span-4 bg-[#C62828] rounded-[2.5rem] p-8 lg:p-10 text-white shadow-2xl relative overflow-hidden group hover:-translate-y-1 transition-all">
